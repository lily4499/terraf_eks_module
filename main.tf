...
resource "aws_vpc" "eks_vpc" {
  cidr_block           = var.vpc_cidr_lil
  enable_dns_support   = var.dns_support_lil
  enable_dns_hostnames = var.dns_hostnames_lil
  tags                 = var.vpc_tags_lil
}
...
resource "aws_subnet" "pub_one" {
  vpc_id                  = aws_vpc.eks_vpc.id
  cidr_block              = var.pub_one_cidr_lil
  availability_zone       = var.az_one_lil
  map_public_ip_on_launch = var.public_ip_launch_lil
...
resource "aws_subnet" "pub_two" {
  vpc_id                  = aws_vpc.eks_vpc.id
  cidr_block              = var.pub_two_cidr_lil
  availability_zone       = var.az_two_lil
  map_public_ip_on_launch = var.public_ip_launch_lil
...
resource "aws_subnet" "priv_one" {
  vpc_id            = aws_vpc.eks_vpc.id
  cidr_block        = var.priv_one_cidr_lil
  availability_zone = var.az_one_lil
...
resource "aws_subnet" "priv_two" {
  vpc_id            = aws_vpc.eks_vpc.id
  cidr_block        = var.priv_two_cidr_lil
  availability_zone = var.az_two_lil
...
#  count          = length(var.pub_subnet_cidr_lil)
...
#  count          = length(var.priv_subnet_cidr_lil)
...
resource "aws_eks_cluster" "lil_cluster" {
  name     = "eks_cluster"
  role_arn = aws_iam_role.eks_cluster_role.arn
  version  = var.eks_version_lil
...
# output "kubeconfig" {
#  value = aws_eks_cluster.lil_cluster.kubeconfig
#}
...
resource "aws_eks_node_group" "eks_node" {
  cluster_name    = aws_eks_cluster.lil_cluster.name
  ...
  scaling_config {
    desired_size = var.desired_size_lil
    max_size     = var.max_size_lil
    min_size     = var.min_size_lil
  }
